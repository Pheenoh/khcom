#include "macros.h"

INCLUDE_ASM("title/task_title_logo_0.s");
INCLUDE_ASM("title/task_title_logo_1.s");
INCLUDE_ASM("title/task_title_logo_2.s");
INCLUDE_ASM("title/task_title_logo_3.s");
INCLUDE_ASM("title/task_title_obj_0.s");
INCLUDE_ASM("title/task_title_obj_1.s");
INCLUDE_ASM("title/task_title_obj_2.s");
INCLUDE_ASM("title/task_title_obj_3.s");
INCLUDE_ASM("title/task_title_menu_0.s");
INCLUDE_ASM("title/func_080D6908.s");
INCLUDE_ASM("title/func_080D6944.s");
INCLUDE_ASM("title/func_080D69AC.s");
INCLUDE_ASM("title/task_title_menu_1.s");
INCLUDE_ASM("title/func_080D6A64.s");
INCLUDE_ASM("title/func_080D6B7C.s");
INCLUDE_ASM("title/func_080D6C54.s");
INCLUDE_ASM("title/func_080D6D2C.s");
INCLUDE_ASM("title/task_title_menu_2.s");
INCLUDE_ASM("title/task_title_menu_3.s");
INCLUDE_ASM("title/task_title_lumichange_0.s");
INCLUDE_ASM("title/task_title_lumichange_1.s");
INCLUDE_ASM("title/task_title_lumichange_2.s");
INCLUDE_ASM("title/task_title_lumichange_3.s");
